...
class MovieQuiz1 extends Component {
    ...
    componentDidMount() {
        const request = new Request('http://127.0.0.1:8080/quiz/movie/1');
        fetch(request)
    ...
    return (
      ...
      {!this.state.submitted && <div>
        <header className="App-header">
          <h4 className="App-title">Movie quiz 1</h4>
        </header>
      ...
      {this.state.submitted &&
          <div>
            <p>Score: {this.state.formData.score}</p>
          </div>
      }
      </div>
    );
